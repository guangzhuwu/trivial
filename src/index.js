...
(() => {
    ...
    let yearOption = `<div class="input-container input-flex-full">
                ...
                <input class="input-range" id="input-years">
              </div>`;
    ...
    let difficultyOption = `<div class="input-container input-flex-full">
                ...
                <input class="input-range" id="input-diff">
              </div>`;
    let difficultyChart = `
    ...
    async function addAnswer(pagename) {
        ...
        if (answer) {
            if (clickedTimes === clickedTimesThen) {
                ...
                $("#answer-button").on('click', async function () {
    ...
    async function addBatchAnswers(pagenames, testName, testYear) {
        ...
        $("#batchans-button").on('click', async () => {
            ...
            $("input[type=radio][name=input-feedback]").on('change', function () {
    ...
    // Sorts
    const sortProblems = (problems) => problems.sort((a, b) => Math.sign(computeYear(a) - computeYear(b)) || computeTest(a).localeCompare(computeTest(b)) || Math.sign(computeNumber(a) - computeNumber(b)));
    ...
    function addProblems(problems, addReplace) {
        ...
        $("#batch-text").before(`<div class="section-options">
                    <button class="text-button section-button" id="copy-problems"
                      data-clipboard-text="${problemList}" title="This list can be copied into ` + `fields for multiple problems, such as making a custom Problem Set.">
                      Copy problem list
    ...
    const formatLatex = (string) => string
        .replace(/&#160;/g, " ")
        .replace(/&#39;/g, "'")
        .replace(/&amp;/g, "&")
        .replace(/&quot;/g, '"')
    ...
    const titleCleanup = (string) => decodeURIComponent(string)
        .replace(/_/g, " ")
    ...
    const sourceCleanup = (string) => string
        .replace(/<span class="fallback-container">.*?<\/span><katex class="katex-container">.*?<annotation encoding="application\/x-tex">(.*?)<\/annotation>.*?<\/katex>/gs, "$$$1$$")
        .replace(/<span class="mw-headline" id="Problem">Problem<\/span><span class="mw-editsection"><span class="mw-editsection-bracket">\[<\/span><a href=".*?" title="Edit section: Problem">edit<\/a><span class="mw-editsection-bracket">]<\/span><\/span><\/h2>/g, "")
    ...
    $("#single-problem").on('click', () => {
        ...
        $("#random-nav").trigger('click');
        $("#random-input").after($(".practice-progress"));
    });

    $("#problem-batch").on('click', () => {
        clearAll();
    ...
    $(".page-container").on("click", "#single-button", async () => {
        ...
        if (!($("#input-singletest").val() + $("#input-singlever").val() in validYears) || $("#input-singleyear").val() < validYears[$("#input-singletest").val() + $("#input-singlever").val()].min || $("#input-singleyear").val() > validYears[$("#input-singletest").val() + $("#input-singlever").val()].max) {
            ...
            await addProblem(sanitize(`${$("#input-singleyear").val()} ${preTest}${$("#input-singletest").val()}${postTest} Problems/Problem ${$("#input-singlenum").val()}`), true);
        }
    });
    ...
    $(".page-container").on("click", "#batch-button", async () => {
        ...
        if (!$("#input-singletest").val()) {
            ...
            $("#solutions-section").remove();
        } else if (!($("#input-singletest").val() + $("#input-singlever").val() in validYears) || $("#input-singleyear").val() < validYears[$("#input-singletest").val() + $("#input-singlever").val()].min || $("#input-singleyear").val() > validYears[$("#input-singletest").val() + $("#input-singlever").val()].max) {
            $(".article-text").before(`<p class="error">
                      The given test is not available for that year.
                    </p>`);
            $(".article-text").remove();
    ...
    $(".page-container").on("click", "#search-button", async () => {
        async function addResults(originalSearch, search, searchResults, pageExists) {
            ...
            if (pageExists) $(".results-notice").append(` | <a href="https://artofproblemsolving.com/wiki/index.php/${encodeURIComponent(underscores(search))}">${capitalize(titleCleanup(encodeURIComponent(originalSearch)))}</a> exists on the wiki`);
                ...
                while (!newProblem && !giveUp) {
                    ...
                    if (newProblem) {
                        ...
                        let problemsList = $("#copy-problems")
                            ...
                            .split(", ");
                        let origOldProblemsList = problemsList.map((e) => e.replace(/_/g, " ").replace("#", "Problems/Problem "));
                        ...
                        let historyIndex = pageHistory.findIndex((e) => e.url === `?problems=${underscores(origOldProblemsList.join("|"))}`);

                        pageHistory[historyIndex].url = `?problems=${underscores(origProblemsList.join("|"))}`;
                        pageHistory[historyIndex].title = name || problemsList
                            .map((e) => titleCleanup(e))
                            .join(", ")
                            .substring(0, 1000) + "...";
    ...
    function collapseText() {
        $("#notes-header").on('click', () => {
            $(".notes").toggleClass("text-collapsed");
        });
        $("#options-header").on('click', () => {
    ...
    async function directLinks() {
        ...
        $("a:not(#aops-wiki-link):not(.aops-link):not(.new)").on('click', async function (event) {
            let href = $(this).attr("href");
            if (href && (href.includes("artofproblemsolving.com/wiki/") || href.includes("?page="))) {
                event.preventDefault();
                let pagename = decodeURIComponent(href
    ...
    function updateYear() {
        ...
        $("#input-singletest").on('change', function () {
        ...
        $("#input-singletest, #input-singlever").on('change', function () {
    ...
    function addHistoryBatch(problems, snippet, title, testYear, testName) {
        let history = JSON.parse(localStorage.getItem("pageHistory"));
        let url = `?problems=${underscores(problems.join("|"))}` + (testYear ? `&testyear=${testYear}&testname=${testName}` : ``);
        let cleanedPage = title || problems
            .map((e) => titleCleanup(e))
            .join(", ")
            .substring(0, 1000) + "...";
        let sanitizedSnippet = sanitize(snippet);
